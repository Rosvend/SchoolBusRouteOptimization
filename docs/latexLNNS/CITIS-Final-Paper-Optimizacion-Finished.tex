%%%%%%%%%%%%%%%%%%%% CITIS-Final-Paper-Optimizacion.tex %%%%%%%%%%%%%%%%%%%%
%
%  Articulo final en la plantilla SNmult (Springer Nature, contributed
%  volume) solicitada para CITIS. Los seis strategies (incluido ALNS con
%  resultados reales) ya estan completos; no quedan marcadores \todo{}
%  activos en el cuerpo (el macro se deja definido por si hace falta en
%  una revision futura). Incluye las tres figuras del benchmark
%  (figures/scalability.pdf, figures/pareto_N400.pdf,
%  figures/comparison_row_N200.png) referenciadas desde una carpeta
%  "figures/" junto a este .tex.
% =====================================================================
...
combinatorial optimization problem generalizing the capacitated
vehicle routing problem and underpinning daily student transport.
Although the literature offers many heuristic strategies, head-to-head
comparisons under identical, realistic conditions remain scarce. We
present a unified benchmark evaluating six clustering-, covering-, and
search-based strategies on identical scenarios from the real
OpenStreetMap road network of Medell\'in (Colombia). All share a solver
interface, directed shortest paths, a single depot, and a bus capacity
of twenty, measured across fleet distance, computation time, coverage,
and cluster cohesion across densities $N\in\{50,100,200,400\}$ with eleven
seeds each (264 runs). Under an identical wall-clock budget, Adaptive
Large Neighborhood Search (ALNS) achieves the shortest fleet distance at
every density, winning all 44 scenarios against clustering pipelines (mean
rank 1.02 of 6) and calibrating within 0.22\% of known CVRP optima.
Crucially, ALNS yields these routes with markedly less cohesive clusters
(silhouette 0.068 vs.\ 0.276 for K-medoids at $N=400$), proving that
spatial compactness is a byproduct of good routes rather than their cause.
Alternatively, zonal CVRPTW caps maximum ride times (reducing longest
routes by 30--47\%), while set-covering models generate feasible plans in
sub-second times per child. We distill these trade-offs into practical
guidance for SBRP solver selection.
...
generalizes the capacitated vehicle routing problem, itself a descendant
of the truck-dispatching problem of Dantzig and
Ramser~\cite{DantzigRamser1959Truck}, and is NP-hard; the intractability
is also combinatorial, since a bus visiting $n_c$ stops admits $n_c!$
orders and the partition across buses must be chosen jointly with those
orders. Practical solvers are therefore overwhelmingly heuristic, and the
surveys of Park and Kim~\cite{Park2010TheReview} and Ellegood et
al.~\cite{Ellegood2020SchoolDirections} show a field fragmented into many
variants whose relative merits are rarely measured on common ground.
...
\emph{measured} per-stage cost, making explicit where two strategies that
look comparable optimize different objectives.
...
set-covering programs hold the speed end of the trade-off. It does so with
the \emph{least} cohesive clusters of the six, which is evidence that
cluster compactness is a means to short routes rather than a proxy for
them (Section~\ref{sec:discussion}).
\end{itemize}
...
distance recovered by changing solver alone. We stop at VKT deliberately:
converting it to emissions needs fleet-specific factors we lack, so that
step is future work rather than an estimate we
...
one or two in isolation. Ellegood et
al.~\cite{Ellegood2020SchoolDirections} confirm over 64 later publications
that single-school, capacity-constrained route generation remains the most
studied core, dominated by heuristics. Wang et
al.~\cite{Wang2025SBRPReview} record a shift towards multi-objective
formulations and learning-based methods, and name what motivates this
paper: the proliferation of variants has outpaced comparative evaluation.
Corber\'an et al.~\cite{Corberan2002HeuristicObjectives} stress that
minimizing buses and minimizing riding time conflict. We adopt the
single-school, capacitated, route-generation variant and report several
competing objectives side by side.
...
report medoid-based clustering as the most robust, a finding our results
...
FasterPAM variant for initialization. The sub-trajectory set-cover
formulation of Akitaya et al.~\cite{Akitaya2021Subtrajectory} links
clustering to set covering, the two paradigms we contrast.
...
metaheuristics are widely applied to routing, including deterministic
annealing~\cite{Baranwal2016DAVRPTW}, the black-hole
algorithm~\cite{Hatamlou2018BlackHole} and genetic
algorithms~\cite{Holland1992Adaptation}, and learning-based approaches
have since matured from graph convolutional
networks~\cite{Joshi2019GCN,Dahiya2018TSPReview} into a distinct family of
neural VRP solvers~\cite{Wu2024NCOSurvey}. A parallel line makes
sustainability explicit in emissions-aware ``green'' VRP
variants~\cite{Sabet2022GreenVRP}. Adaptive large-neighbourhood search
enters our benchmark as the locality-aware representative of the
metaheuristic family (Section~\ref{sec:alns}), though a single
configuration does not bound that family.
...
weights $w_e>0$, with a single school (depot) $o\in V$, $n$ student stops
$\mathcal{C}=\{1,\dots,n\}\subset V$ and a homogeneous fleet of capacity
$C$. The road distance $d_{ij}$ is the shortest-path cost
$\min_{p\in\mathcal{P}(i,j)}\sum_{e\in p} w_e$ over directed paths
$\mathcal{P}(i,j)$, and is in general asymmetric
($d_{ij}\neq d_{ji}$). Clustering algorithms that assume a metric use the
symmetrized $D^{\mathrm{sym}}_{ij}=\tfrac{1}{2}(d_{ij}+d_{ji})$, whereas
the routing stage uses the directed $d_{ij}$.
...
\textstyle\sum_{i\in S}\sum_{j\in S} y_{ij}\le |S|-1
  ...
  \label{eq:dfj}
\end{equation}
which forbids any proper subset from carrying $|S|$ internal arcs and so
rules out cycles that do not span $V_c$. Having exponentially many
...
Search~\cite{Voudouris1999GLS} under a fixed budget, so
(\ref{eq:dfj}) defines the optimum we approximate, not the algorithm.
...
and scored by identical metric code. The framework uses
...
scenario generator is deterministic given $(N,\text{seed})$. This
interface is what lets the ALNS adapter enter the comparison without
...
sweep packs students into capacity-bounded sectors, refined by the same
minimum-cost-flow reassignment as the baseline, giving compact
wedge-shaped clusters radiating from the school.
...
none of them. Eq.~(\ref{eq:setcover}) is a \emph{covering} ($\ge1$) rather
than \emph{partitioning} ($=1$) constraint and routes run as generated, so
a student in two selected candidates is visited twice and both visits are
charged: at $N=400$ per-child selects $26.9$ capacity-filled routes, $538$
seats for $400$ students, making about a quarter of visits redundant.
$\mathcal{R}$ is also a heuristically seeded subset of all
$\binom{n}{\le C}$ feasible routes, so the branch-and-cut speed reflects a
...
the other's pool, which we leave to future work.
...
same \texttt{Solver} interface, so an adaptive search is measured on the
same scenarios and metrics as the other five.

It starts from a feasible solution and repeatedly \emph{destroys} and
...
and ALNS $O(I\,n)$--$O(I\,n\log n)$ depending on the operator. Those
bounds are not what governs wall-clock time, so we timed the stages
instead ($n$ students, $k=\lceil n/C\rceil$ buses, $T$ the iteration cap).

We timed the stages rather than assuming. For K-medoids and Sectorial the
minimum-cost-flow assignment takes $0.04\%$ of solver time at $N=50$ and
$1.1\%$ at $N=400$ ($1.09$ of $101.13$~s), with FasterPAM under $0.02$~s;
the per-cluster TSP takes the rest and is \emph{budget-bound}, costing
almost exactly $k\cdot B$ ($15.03$, $25.01$, $50.01$, $100.03$~s for
$k=3,5,10,20$ at $B=5$~s), so latency grows linearly in $n$. The
$O(T\,n^3)$ flow bound is real but latent and dominates only far beyond
the sizes studied. ALNS is budget-bound by construction rather than by
inheritance, so its timers answer how many iterations the budget buys:
repair takes $65$--$71\%$ of every run against $17$--$29\%$ for destroy,
because each insertion is scored over every route and position, an
$O(k\,|r|)$ sweep per removed student, whereas removal needs one pass.
Initial construction never exceeds $1.1\%$. Since the budget grows as
$k\cdot B$ while per-iteration cost grows with $n$, the realised iteration
count falls only from $15{,}095$ to $10{,}353$ as $n$ grows eightfold.

\begin{figure}[t]
\centering
\includegraphics[width=0.68\textwidth]{figures/scalability.pdf}
\Description{Log-log line plot of mean latency in seconds against
number of students N for six strategies. K-medoids, Sectorial and ALNS
trace an almost identical rising line; the zonal CVRPTW is flat at its
fixed per-zone budget; the two set-covering variants start well below
one second and rise with N, the grid variant rising fastest.}
\caption{Mean latency vs.\ $N$ (log--log). K-medoids, Sectorial and
ALNS trace the same curve because all three spend the matched
$k\cdot5$~s wall-clock budget of Section~\ref{sec:alns}; the zonal
CVRPTW is pinned to its fixed per-zone budget; the set-covering
variants start orders of magnitude faster and grow with the size of
their candidate pool and integer program.}
...
service-zone polygons, so every instance stays within the actual
metropolitan footprint. Directed shortest-path distance and travel-time
matrices are precomputed with Dijkstra's algorithm; bus capacity is
$C=20$.

We sweep $N\in\{50,100,200,400\}$ with eleven seeds each, giving $44$
scenarios and $264$ runs. Per run we record fleet distance, wall-clock
latency, buses used, coverage, the silhouette
coefficient~\cite{Rousseeuw1987Silhouettes} of the clustering on the
symmetrized distances, capacity violations and the longest single route.
To keep latency comparable the per-cluster TSP is capped at $5$~s and the
per-zone CVRPTW at $20$~s. All strategies run at fixed settings with no
per-instance tuning: K-medoids uses $T=50$; Sectorial $\alpha=0.6$,
$\beta=0.4$ and $T=5$; the zonal CVRPTW $75$~min maximum duration, $90$~s
boarding and span coefficient $300$; ALNS the parameters of
Section~\ref{sec:alns} under its matched $k\cdot5$~s budget. Significance
across strategies is assessed with the Friedman test (blocks $=$ the $44$
scenarios) and pairwise differences with the Wilcoxon signed-rank test.
...
establishes relative standings but says nothing about absolute quality. We
therefore run the ALNS solver, unchanged and through the same
\texttt{Solver} interface, on the $27$ instances of Augerat
set~A~\cite{Augerat1995CVRP}, a classical CVRP family with published
optima, $n=31$ to $79$ and capacity $100$. The loader reproduces the
TSPLIB \texttt{EUC\_2D} convention, integer
$\mathrm{nint}(\sqrt{\Delta x^2+\Delta y^2})$, verified by recomputing
each published optimal solution under our matrix and recovering its
published cost for all $27$. Two differences matter: these instances carry non-unit demands $q_i$, so
capacity is a demand sum rather than a stop count, and each declares a
vehicle count equal to $\lceil \sum_i q_i / C\rceil$, so ALNS optimizes
over the same fleet size the optimum uses. Each gets $60$~s. Only ALNS is
run: the clustering pipelines assume unit demand, under which their
assignment is an exact minimum-cost flow, and generalising them would
change the algorithms compared.

% =====================================================================
\section{Results}\label{sec:results}
% =====================================================================
Table~\ref{tab:fleet} reports fleet distance and latency, generated
mechanically from the run output; Figure~\ref{fig:pareto} plots the
same two metrics jointly at $N{=}400$ and Figure~\ref{fig:scalability}
shows how latency scales with $N$ for all six strategies. At the most demanding density
...
\begin{figure}[t]
\centering
\includegraphics[width=0.68\textwidth]{figures/pareto_N400.pdf}
...
\caption{Fleet distance vs.\ latency at $N{=}400$ (lower-left is
better). Within the same wall-clock budget as the clustering pipelines,
ALNS reaches the shortest distance of all six strategies; the
set-covering variants trade distance for speed at two different points
of that trade-off; K-medoids and Sectorial are statistically
indistinguishable, their $1.0$~km gap being inside seed noise.}
\label{fig:pareto}
\end{figure}
...
$0.221\%$ above the optimum, with median gap exactly $0$; every solution
is capacity-feasible, covers all customers and uses the declared vehicle
...
and the count of proven optima at $19$. What remains is a limit of the
operator pool, not of the time given to it. This does not make our ALNS state-of-the-art, since dedicated solvers
close set~A routinely, but it bounds what the rest of the paper leaves
open: the strategy that wins our benchmark is within a fraction of a
percent of optimal on independent instances, so the margins in
Table~\ref{tab:fleet} separate a near-optimal method from the rest rather
than six equally unvalidated ones.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{figures/comparison_row_N200.png}
...
\caption{Three of the six strategies on the same scenario ($N{=}200$,
seed 1). Left: Sectorial's wedge-shaped clusters. Centre: the grid
set-covering ILP, whose selected routes overlap near the depot because
Eq.~(\ref{eq:setcover}) only requires coverage, not a partition, and
needs one more bus (11 vs.\ 10) to do so. Right: the ALNS solution,
matching Sectorial's fleet size with a visibly less compact partition,
consistent with the cohesion results discussed below.}
\label{fig:grid}
\end{figure}
...
medoid optimization (Wilcoxon $p=0.36$).
...
$0.174$, $0.068$) even as its advantage holds at $4.8$--$7.3\%$. Cohesion
and cost are not two views of one quantity. A compact partition is a good
\emph{heuristic} for short routes because it makes each TSP easy, but it
is a constraint on the solution, and a search optimizing
...
the cohesion--cost correlation. Within a density, silhouette and fleet
...
benchmark; with ALNS in its place the same computation gives $r=+0.09$
($p=0.48$), and no density shows a significant negative association. It
was carried by one strategy that was at once incoherent and long, not by a
law relating the two, so we report cohesion as descriptive rather than
predictive.
...
longest route ($30.3$~km) is $30$--$47\%$ shorter than any other
method's, because the span-balancing penalty and
the explicit time dimension cap individual ride times. Where bounded ride
time is a hard requirement, it is the only strategy that delivers it
directly.
...
distance they take the last two places of the six (mean ranks $5.05$ and
$5.95$); on latency the first two ($2.18$ and $1.00$). Their route quality
should not be compared with each other, however, for the reasons given in
Section~\ref{sec:setcover-two}: they optimize different objectives over
differently built pools, so the gap between them mixes two effects. Grid's
latency also turns erratic at $N=400$ ($66.6\pm34.7$~s) as the pool, and
hence the integer program, grows. Both remain right when a feasible plan
is needed in near-real-time.

\paragraph{Limitations.}
Eleven seeds per density give $44$ blocks and $\binom{6}{2}=15$ pairwise
comparisons, of which the Nemenyi critical difference separates $11$ on
fleet distance. Four remain open: K-medoids versus Sectorial
($\Delta=0.045$), each of those against the zonal CVRPTW ($1.01$ and
$1.06$, just under $\mathrm{CD}=1.14$), and the two set-covering variants
against each other ($0.91$). The first is a genuine equivalence by every
test we applied; the other three are power limits of a $44$-block design,
not evidence of equivalence. Every comparison involving ALNS is resolved.

The external reference point is partial rather than absent.
Section~\ref{sec:external} calibrates only \emph{one} of the six
strategies and only against published \emph{CVRP} optima, so three gaps
remain: the other five are uncalibrated in absolute terms and their
standings rest on the comparison with ALNS; set~A is small ($n\le79$) and
Euclidean whereas our scenarios are larger and road-network based, so
near-optimality there need not carry to $N=400$ in Medell\'in; and we
compare against published \emph{values}, not other authors' running
implementations, so we can state distance from optimal but not relative
runtime. Our results also speak to \emph{one} ALNS configuration, so its ranking is
not a bound on what the method can achieve. Finally, the benchmark models
a single school with static demand and a homogeneous fleet, and all
strategies share one budget, so absolute latencies reflect that budget
rather than an intrinsic bound.
...
The central finding is that an adaptive large-neighbourhood search,
given exactly the wall clock the clustering pipelines receive, is the
strongest strategy we measured: shortest fleet distance in all $44$
scenarios, $4.8$--$7.3\%$ below the previous leaders and separated from
them by more than the Nemenyi critical difference, with calibration
against $27$ published CVRP instances placing it a mean $0.22\%$ from the
known optima ($19$ solved to optimality). Capacitated k-medoids and
sectorial clustering remain tied and are the best cluster-first pipelines;
set-covering programs hold the fast end of the trade-off at two different
points of it, because they optimize different objectives; and a zonal
CVRPTW is the choice when bounded ride time is mandatory.

The result we did not expect is that ALNS wins with the \emph{least}
cohesive partition of the six. Cluster compactness is a means to short
routes, not a proxy for them, and the strong cohesion--cost correlation
reported for an earlier version of this benchmark does not survive
substituting a locality-aware search for a topology-agnostic one.

Future work will (i)~extend the external calibration to the remaining five
strategies, to larger road-network-based instances, and to running
comparisons against other authors' implementations rather than published
values alone; (ii)~broaden the statistical design to resolve the pairs the
Nemenyi test leaves open; (iii)~vary the ALNS operator mix, acceptance
schedule and budget, which Section~\ref{sec:external} shows is where its
residual gap lies rather than in the time given to it; (iv)~extend the
harness to multi-school and dynamic-demand variants; and (v)~convert the
vehicle-kilometre results into emissions and operating cost using
...
% El propio archivo de la plantilla usa minuscula: \input{references}
% (sensible a mayusculas en Linux/Overleaf). Este .tex ya provee el
% entorno thebibliography, asi que no se debe usar tambien
