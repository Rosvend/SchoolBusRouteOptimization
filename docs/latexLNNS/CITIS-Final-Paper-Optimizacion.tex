...
practitioners to choose a method. We present a unified benchmark evaluating six clustering-, covering- and
search-based strategies: capacitated k-medoids, a multi-objective
sectorial clustering, a zonal capacitated vehicle routing problem with
time windows (CVRPTW), two set-covering integer programs and an adaptive
large-neighbourhood search (ALNS), on identical scenarios from the real
OpenStreetMap road network of Medell\'in (Colombia). All share a solver
interface, directed shortest-path distances, a single depot and a bus
capacity of twenty, and are measured on fleet distance, computation time,
fleet size, coverage and cluster cohesion across densities
$N\in\{50,100,200,400\}$ with eleven seeds each (264 runs). Capacitated k-medoids and sectorial
...
mechanically from the run output. At the most demanding density
...
% The camera-ready template includes the bibliography manually via
% \input{References}, which already provides the thebibliography
% environment. Do not also use \bibliographystyle/\bibliography: that
% duplicates the list and breaks the build under bibtex/latexmk.
\input{References}
