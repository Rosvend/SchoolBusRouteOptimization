...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    Buses utilizados         & \textit{pend.} & \textit{pend.}\\
    Distancia total (km)     & \textit{pend.} & \textit{pend.}\\
    Distancia media/bus (km) & \textit{pend.} & \textit{pend.}\\
    Tama\~no m\'in.\ (est.)  & \textit{pend.} & \textit{pend.}\\
    Tama\~no m\'ax.\ (est.)  & \textit{pend.} & \textit{pend.}\\
    Tama\~no promedio (est.) & \textit{pend.} & \textit{pend.}\\
    \bottomrule
  \end{tabular}\\[4pt]
  \small\textit{Completar con los valores de \texttt{src/comparacion.ipynb}
  tras ejecutar la pipeline completa.}
\end{table}
