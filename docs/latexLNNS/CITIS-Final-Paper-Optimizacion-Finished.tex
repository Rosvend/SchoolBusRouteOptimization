...
interface, directed shortest paths, a single depot, and a bus capacity of
twenty, measured on fleet distance, computation time, coverage, and
cluster cohesion across densities $N\in\{50,100,200,400\}$ with eleven
...
every density, winning all 44 scenarios against the clustering pipelines
(mean rank 1.02 of 6) and calibrating within 0.22\% of known CVRP optima.
Crucially, ALNS yields these routes with markedly less cohesive clusters
(silhouette 0.068 vs.\ 0.276 for K-medoids at $N=400$), indicating that
spatial compactness is a byproduct of good routes rather than their cause.
In contrast, the zonal CVRPTW caps ride time, cutting the longest route by
30--47\% at $N{=}400$, while the two set-covering programs trade distance
for speed at two different points of that trade-off because they optimize
different objectives. We distill these trade-offs into practical
...
set-covering programs hold the speed end of the trade-off --- and it does
so with the \emph{least} cohesive clusters of the six, evidence that
...
needs fleet-specific factors we lack, so we stop there rather than invent
an estimate~\cite{Sabet2022GreenVRP}.
...
ALNS adapter enter the comparison without touching the scenarios, metrics
...
and ALNS $O(I\,n)$--$O(I\,n\log n)$ depending on the operator ($n$
students, $k=\lceil n/C\rceil$ buses, $T$ the iteration cap). Those bounds
are not what governs wall-clock time, so we timed the stages instead. For
K-medoids and Sectorial the minimum-cost-flow assignment takes $0.04\%$ of solver time at $N=50$ and
...
$O(T\,n^3)$ flow bound is latent and dominates only far beyond the sizes
studied. ALNS is budget-bound by construction, so its timers answer how
many iterations the budget buys: repair takes $65$--$71\%$ of every run against $17$--$29\%$ for destroy,
...
run the ALNS solver, unchanged and through the same \texttt{Solver}
interface, on the $27$ instances of Augerat
...
and the proven optima at $19$ --- a limit of the operator pool, not of the
time given to it. Dedicated solvers close set~A routinely, so this does
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.76\textwidth]{figures/comparison_row_N200.png}
...
and road-network based, so near-optimality there need not carry to $N=400$
in Medell\'in; and we compare against published \emph{values}, not other
