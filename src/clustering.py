...
#  Capacitated assignment via min-cost flow 
def capacitated_assign(D, medoids, capacity):
    """Assign children to medoids respecting capacity via min-cost flow."""
    n = D.shape[0]
    G = nx.DiGraph()

    # source → each child (capacity 1)
    for i in range(n):
        G.add_edge("s", ("c", i), capacity=1, weight=0)

    # each child → each medoid (capacity 1, cost = distance in mm)
    for i in range(n):
        for m_idx, m in enumerate(medoids):
            cost = int(D[i, m] * 1000)  # mm precision for integer requirement
            G.add_edge(("c", i), ("m", m_idx), capacity=1, weight=cost)

    # each medoid → sink (capacity = bus_capacity)
    for m_idx in range(len(medoids)):
        G.add_edge(("m", m_idx), "t", capacity=capacity, weight=0)

    # demands: source supplies n, sink consumes n
    G.nodes["s"]["demand"] = -n
    G.nodes["t"]["demand"] = n
...
def recompute_medoids(D, labels, k):
    """For each cluster, pick the point minimizing total within-cluster distance."""
    medoids = []
    for c in range(k):
